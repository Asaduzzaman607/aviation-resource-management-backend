...
@RestController
@RequestMapping("/api/store-inspector/quality/inspection-checklists")
public class InspectionCheckListQualityController {
    private final InspectionChecklistService inspectionChecklistService;


    public InspectionCheckListQualityController(InspectionChecklistService inspectionChecklistService) {
    ...
    public ResponseEntity<MessageResponse> updateActiveStatus(@PathVariable Long id, @RequestParam("active") Boolean isActive) {
        inspectionChecklistService.updateActiveStatus(id, isActive, VendorWorkFlowType.QUALITY);
    ...
    ApprovalRequestDto approvalRequestDto) {
        inspectionChecklistService.makeDecision(id, approvalRequestDto, VendorWorkFlowType.QUALITY);
