...
@RestController
@RequestMapping("/api/logistic/comparative-statements/audit-management")
public class LogisticCsAuditWorkflowController {
    ...
    public LogisticCsAuditWorkflowController(CsUtilService csUtilService, CsAuditDisposalService csAuditDisposalService,
    ...
    public ResponseEntity<MessageResponse> makeDecisionToAudit(@PathVariable Long id, @Valid @RequestBody ApprovalRequestDto approvalRequestDto) {
        csUtilService.decision(id, approvalRequestDto, CsWorkflowType.AUDIT, RfqType.LOGISTIC);
    ...
    public ResponseEntity<MessageResponse> updateActiveStatus(@PathVariable Long id, @RequestParam("active") Boolean isActive) {
        ...
        return ResponseEntity.ok(new MessageResponse(ApplicationConstant.STATUS_CHANGED));
    }
