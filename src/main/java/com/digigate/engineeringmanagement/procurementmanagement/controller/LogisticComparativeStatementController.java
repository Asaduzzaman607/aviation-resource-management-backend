...
@RestController
@RequestMapping("/api/logistic/comparative-statements/material-management")
public class LogisticComparativeStatementController {
    ...
    public LogisticComparativeStatementController(ComparativeStatementService comparativeStatementService, CsUtilService csUtilService) {
    ...
    public ResponseEntity<CsResponseDto> generateCs(@Valid @RequestBody CsGenerateDto csGenerateDto) {
        csGenerateDto.setRfqType(RfqType.LOGISTIC);
        return ResponseEntity.ok(comparativeStatementService.generateCs(csGenerateDto, OrderType.PURCHASE));
    }

    @PostMapping
    public ResponseEntity<MessageResponse> create(@Valid @RequestBody CsDto csDto) {
        csDto.setRfqType(RfqType.LOGISTIC);
    ...
    public ResponseEntity<MessageResponse> makeDecision(@PathVariable Long id, @Valid @RequestBody ApprovalRequestDto approvalRequestDto) {
        csUtilService.decision(id, approvalRequestDto, CsWorkflowType.CS_INITIAL, RfqType.LOGISTIC);
